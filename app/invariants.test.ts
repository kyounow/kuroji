...
import { balances, totalAssets, totalLiabilities, totalEquity, developmentAssetOf } from '@core/index'
...
/** 1ターン記録＋直前状態から不変条件違反を列挙（空配列＝健全）。params を渡すと商材開発の整合も検証。 */
function checkRecord(
  rec: GameState['history'][number],
  ...
  if (params?.devProjects?.length) {
    ...
    for (const w of s.devInProgress ?? []) {
      const proj = params.devProjects.find((d) => d.id === w.projectId)
      if (proj && w.invested > proj.requiredInvestment) p.push(`${ctx}: WIP投資が必要額超過`)
      if (w.invested < 0) p.push(`${ctx}: WIP投資<0`)
    }
    for (const d of s.devLaunched ?? []) {
      if (d.bookValue < 0) p.push(`${ctx}: 開発資産簿価<0`)
    }
  }
  return p
}
...
function randomDecision(game: GameState, params: SimParams, seed: number, turn: number, sink: boolean): Decision {
  ...
  return {
    ...
    lines: params.productLines?.length
      ...
      : undefined,
    // 商材開発への投資（資産計上/費用処理・ローンチ・償却・陳腐化の経路をストレス）。
    devSpend: params.devProjects?.length
      ? Object.fromEntries(
          params.devProjects.map((dp, i) => [
            dp.id,
            sink || u(seed, turn, 60 + i * 3) < 0.3
              ? Math.round(dp.requiredInvestment * 0.4 * u(seed, turn, 61 + i * 3))
              : 0,
          ]),
        )
      : undefined,
    ...
    for (const sc of AVAILABLE_SCENARIOS) {
      ...
      for (let seed = 1; seed <= 8; seed++) {
        ...
        for (let t = 0; t < 120 && game.outcome === 'playing'; t++) {
          ...
          if (rec) problems.push(...checkRecord(rec, prev, `${sc.id}/seed${seed}/t${t}`, scenario.params))
    ...
    for (const sc of AVAILABLE_SCENARIOS) {
      ...
      for (let seed = 1; seed <= 4; seed++) {
        ...
        for (let t = 0; t < 120 && game.outcome === 'playing'; t++) {
          ...
          const rec = game.history[game.history.length - 1]
          if (rec) problems.push(...checkRecord(rec, prev, `sink:${sc.id}/seed${seed}/t${t}`, scenario.params))
          if (problems.length > 20) break
        }
        if (problems.length > 20) break
      }
...
describe('不変条件C: 倒産経路と資本注入エクスプロイト回帰', () => {
  it('悪手を続ければ倒産に到達し、最終状態も恒等式維持', () => {
    ...
    for (let t = 0; t < 200; t++) {
      ...
      if (rec) expect(checkRecord(rec, prev, `bankrupt/t${t}`, getScenario('cafe').params)).toEqual([])
      if (g.outcome === 'lost') {
        reached = true
        break
      }
